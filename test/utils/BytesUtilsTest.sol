...
contract BytesUtilsTest {
    ...
    function testReadBytes() public {
        ...
        Assert.equal(
            bytesUtils.pReadBytes32(testData, 0),
            bytes32(uint256(123)),
            "Read index 0, uint256"
        );
        Assert.equal(
            bytesUtils.pReadBytes32(testData, 1),
            bytes32(address(this)),
            "Read index 1, address"
        );
        Assert.equal(
            bytesUtils.pReadBytes32(testData, 2),
            keccak256("test"), "Read index 2, bytes32");
        Assert.equal(
            bytesUtils.pReadBytes32(testData, 3),
            bytes32(0x789), "Read index 3, bytes32"
        );
    }
    ...
    function testReadNonBytesMemory() public {
        ...
        Assert.isFalse(
            ...
            "Read index 4 should fail, it's not inside bytes array"
        );
        ...
        Assert.isFalse(
            throwProxy.execute.gas(200000)(),
            "Read index 0 should fail, it's not inside bytes array"
        );
    }
    ...
    function testInvalidLengthBytes() public {
        ...
        Assert.equal(
            bytesUtils.pReadBytes32(testData, 1),
            bytes32(address(this)), "Read index 1, address"
        );
        ...
        Assert.isFalse(
            throwProxy.execute.gas(200000)(),
            "Reading index 3 should fail, the word has less than 32 bytes"
        );
    }
}
